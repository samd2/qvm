...
(C) 2008-2021 Emil Dotchevski and Reverge Studios, Inc.
...
The source code is available in the https://github.com/boostorg/qvm[QVM GitHub repository].
...
* link:qvm.hpp[qvm.hpp]: single header containing all QVM code, including complete swizzling overloads (huge).
* link:qvm_lite.hpp[qvm_lite.hpp]: single header containing everything except for the swizzling overloads.

== Portability

Boost QVM requires only {CPP}03 but is tested on many compiler versions and {CPP} standards.
